...
function ents.PFMVRController:OnRemove()
	if util.is_valid(self.m_guiInterface) then
		self.m_guiInterface:Remove()
	end
	if util.is_valid(self.m_guiEnt) then
		self.m_guiEnt:Remove()
	end
end

function ents.PFMVRController:GetGUIElement()
	return self.m_guiInterface
end
...
function ents.PFMVRController:InitializeInterface()
	...
	if gui3dC ~= nil then
		gui3dC:SetGUIElement(self.m_guiInterface)
	end
	self.m_guiEnt:Spawn()
