...
function pfm.ComponentManager:LoadComponentsFile(fileName, binary)
	...
	if f == nil then
		pfm.log(
			"Unable to load component config: File '" .. fileName .. "' not found!",
	...
	if not udmComponents:IsValid() then
		return true
	end
	if self.m_udmComponents == nil then
		self.m_udmComponents = udmComponents:ClaimOwnership()
	else
		self.m_udmComponents:Get():Merge(udmComponents)
	end
	return true
end
...
function pfm.ComponentManager:GetComponents()
	return self.m_udmComponents and self.m_udmComponents:Get() or udm.LinkedPropertyWrapper()
end
