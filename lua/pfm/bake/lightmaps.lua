...
pfm.bake.find_bake_entities = function()
	...
	for _, actor in ipairs(actors) do
		...
		if
			util.is_valid(ent)
			and entMap[ent] == nil
			and ent:IsInert() == false
			and ent:HasComponent(ents.COMPONENT_PFM_MODEL)
			and ent:HasComponent(ents.COMPONENT_RENDER)
		then
	...
	for ent, c in ents.citerator(ents.COMPONENT_MAP, { ents.IteratorFilterComponent(ents.COMPONENT_MODEL) }) do
		if ent:HasComponent(ents.COMPONENT_PFM_ACTOR) == false and ent:IsInert() then
			...
			if physC == nil or physC:GetPhysicsType() == phys.TYPE_STATIC then
				if
					ent:HasComponent(ents.COMPONENT_LIGHT_MAP_RECEIVER) or ent:HasComponent(ents.COMPONENT_LIGHT_MAP)
				then
					entMap[ent] = true
				else
					influencerMap[ent] = true
	...
	entities = {}
	for ent, _ in pairs(entMap) do
		table.insert(entities, ent)
	end
	local influencers = {}
	for ent, _ in pairs(influencerMap) do
		table.insert(influencers, ent)
	end
	return entities, influencers
end

pfm.bake.find_bake_light_sources = function()
	return ents.get_all(ents.citerator(
...
pfm.bake.lightmaps = function(
	...
	local scene = unirender.create_scene(
		bakeCombined and unirender.Scene.RENDER_MODE_BAKE_DIFFUSE_LIGHTING
			or unirender.Scene.RENDER_MODE_BAKE_DIFFUSE_LIGHTING_SEPARATE,
		createInfo
	)
	...
	if lightmapDataCache ~= nil then
		scene:SetLightmapDataCache(lightmapDataCache)
	end -- Has to be set before adding any bake targets!

	local sceneFlags = unirender.Scene.SCENE_FLAG_NONE
	local influencerMap = {}
	for _, ent in ipairs(influencers) do
		influencerMap[ent] = true
	end
	...
	if renderer == false then
		pfm.log(
			"Unable to create renderer for render engine '" .. createInfo.renderer .. "': " .. err .. "!",
...
function LightmapBaker:IsComplete()
	return self.m_job:IsComplete()
end
function LightmapBaker:IsSuccessful()
	return self.m_job:IsSuccessful()
end
function LightmapBaker:GetProgress()
	return self.m_job:GetProgress()
end
function LightmapBaker:GetResult()
