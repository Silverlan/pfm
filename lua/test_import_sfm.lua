...
local function initialize_track(pfmScene,sfmTrack)
	...
	for _,soundClip in ipairs(sfmTrack:GetSoundClips()) do
		...
		--[[track:SetTest(5.0)
		print("Test: ",track:GetTest())
		
		local clip = udm.create_attribute(udm.ELEMENT_TYPE_PFM_CLIP)
		track:AddClip(clip)]]
	end
end
  ...
  for _,clip in ipairs(clips) do
    ...
    if(subClipTrackGroup ~= nil) then
      for _,track in ipairs(subClipTrackGroup:GetTracks()) do
        iterate_clips(track:GetFilmClips())
        --for _,clip in ipairs(track:GetSoundClips()) do
        --  print(clip:GetType())
        --end
      end
    end]]
