...
function util.GraphAxis:__init()
	...
	local fOnPropsChanged = function()
		self:CallCallbacks("OnPropertiesChanged")
	end
...
function util.GraphAxis:SetZoomLevelLimits(min, max)
	self.m_zoomLevelLimits = { min, max }
end
function util.GraphAxis:GetZoomLevelLimits()
	return self.m_zoomLevelLimits
end
...
function util.GraphAxis:GetZoomLevel()
	return self.m_zoomLevel:Get()
end
function util.GraphAxis:GetZoomLevelProperty()
	return self.m_zoomLevel
end
function util.GraphAxis:GetZoomLevelMultiplier()
	return self:GetZoomLevelMultiplier()
end
function util.GraphAxis:GetZoomLevelMultiplierFraction()
	return 10 ^ self:GetZoomLevel()
end
function util.GraphAxis:GetZoomLevelMultiplier()
	return 10 ^ math.ceil(self:GetZoomLevel())
end
function util.GraphAxis:SetStartOffset(offset)
	self.m_startOffset:Set(offset)
end
function util.GraphAxis:GetStartOffset()
	return self.m_startOffset:Get()
end
function util.GraphAxis:GetStartOffsetProperty()
	return self.m_startOffset
end
function util.GraphAxis:GetStridePerUnit()
	return (1.0 / self:GetZoomLevelMultiplierFraction()) * self.m_strideInPixels
end
function util.GraphAxis:SetStrideInPixels(stride)
	self.m_strideInPixels = stride
end
function util.GraphAxis:GetStrideInPixels()
	return self.m_strideInPixels
end
