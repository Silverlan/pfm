...
function ents.ParticleSystemComponent.OperatorAlphaFade:Simulate(pt, dt)
	...
	if age < 1.0 then
		if age >= self.m_startFadeInTime and age <= self.m_endFadeInTime then
			...
			local newAlpha = (fadeInDuration > 0.0)
					and SimpleSplineRemapValWithDeltasClamped(
						age,
						self.m_startFadeInTime,
						fadeInDuration,
						fadeInDurationOO,
						goal,
						(initialAlpha - goal)
		...
		if age >= self.m_startFadeOutTime and age <= self.m_endFadeOutTime then
			...
			local newAlpha = (fadeOutDuration > 0.0)
					and SimpleSplineRemapValWithDeltasClamped(
						age,
						self.m_startFadeOutTime,
						fadeOutDuration,
						fadeOutDurationOO,
						initialAlpha,
						(goal - initialAlpha)
...
ents.ParticleSystemComponent.register_operator(
	...
	ents.ParticleSystemComponent.OperatorAlphaFade
)
