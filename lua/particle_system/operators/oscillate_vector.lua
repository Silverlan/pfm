...
function ents.ParticleSystemComponent.OperatorOscillateVector:Simulate(pt, dt, strength)
	...
	if lifeDuration > 0.0 and lifeTime >= startTime and lifeTime < endTime then
		...
		if self.m_proportional then
			lifeTime = age * ReciprocalEst(lifeDuration)
			cos = ((lifeTime * frequency) * self.m_oscillationMultiplier)
				+ Vector(self.m_oscillationStartPhase, self.m_oscillationStartPhase, self.m_oscillationStartPhase)
		else
			local curTime = pt:GetTimeCreated() + age
			local cosFactor = (self.m_oscillationMultiplier * curTime) * Vector(1, 1, 1)
				+ Vector(self.m_oscillationStartPhase, self.m_oscillationStartPhase, self.m_oscillationStartPhase)
		...
		if curVal ~= nil and util.get_type_name(curVal) == "Vector4" then
			local oscVal = Vector(curVal.x, curVal.y, curVal.z)
				+ oscMultiplier * Vector(SinEst01(cos.x), SinEst01(cos.y), SinEst01(cos.z))
...
ents.ParticleSystemComponent.register_operator(
	...
	ents.ParticleSystemComponent.OperatorOscillateVector
)
