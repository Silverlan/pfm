...
util.register_class(
	"ents.ParticleSystemComponent.InitializerPositionWithinSphere",
...
function ents.ParticleSystemComponent.InitializerPositionWithinSphere:Initialize()
	...
	self.m_distanceBiasAbsoluteValue =
		vector.create_from_string(self:GetKeyValue("distance_bias_absolute_value") or "0 0 0")
	...
	self.m_speedInLocalCoordinateSystemMin =
		vector.create_from_string(self:GetKeyValue("speed_in_local_coordinate_system_min") or "0 0 0")
	self.m_speedInLocalCoordinateSystemMax =
		vector.create_from_string(self:GetKeyValue("speed_in_local_coordinate_system_max") or "0 0 0")
	self.m_createInModel = tonumber(self:GetKeyValue("create_in_model")) or 0
	self.m_randomlyDistributeToHighestSuppliedControlPoint =
		toboolean(self:GetKeyValue("randomly_distribute_to_cp") or "0")
	self.m_randomlyDistributionGrowthTime = tonumber(self:GetKeyValue("random_distribution_growth_time")) or 0

	self.m_hasDistanceBias = (
		self.m_distanceBias.x ~= 1.0
		and self.m_distanceBias.y ~= 1.0
		and self.m_distanceBias.z ~= 1.0
	)
	self.m_hasDistanceBiasAbs = (
		self.m_distanceBiasAbsoluteValue.x ~= 0.0
		and self.m_distanceBiasAbsoluteValue.y ~= 0.0
		and self.m_distanceBiasAbsoluteValue.z ~= 0.0
	)
end
...
function ents.ParticleSystemComponent.InitializerPositionWithinSphere:OnParticleCreated(pt)
	...
	if self.m_speedMax > 0.0 then
		local randSpeed = self.m_speedMin
			+ (math.randomf(0.0, self.m_speedMax - self.m_speedMin) ^ self.m_speedRandomExponent)
	...
	if renderer == nil or renderer:GetName() ~= "source_render_sprite_trail" then
		return
	end -- TODO: Find a better way to do this!
