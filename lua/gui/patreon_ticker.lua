...
function Element:AddSupporter(name,color,priority)
	...
	if(color ~= nil) then time.create_simple_timer(0.0,function() if(elText:IsValid()) then elText:SetColor(color) end end) end
	self.m_marquee:AddElement(elText)
end
function Element:OnSizeChanged(w,h)
	if(util.is_valid(self.m_marquee)) then self.m_marquee:SetWidth(w) end
end
...
function gui.PatreonTicker:OnInitialize()
	...
	local patronTicker = gui.create("WISupporterTicker",self,patronTickerLabel:GetWidth(),0,self:GetWidth() -patronTickerLabel:GetWidth(),self:GetHeight())
...
function gui.PatreonTicker:UpdateTicker()
	for _,el in ipairs(self.m_patronTicker.m_marquee:GetElements()) do
		if(el:IsValid()) then el:SizeToContents() end
	end
...
function gui.PatreonTicker:OnThink()
	...
	if(query ~= nil and query:IsSuccessful()) then
		...
		for i,patron in ipairs(patrons) do
			...
			local color
			if(patronInfo[2] ~= nil) then color = Color.CreateFromHexColor(patronInfo[2]:sub(2)) end
	...
	self.m_patronTicker:Update()
	time.create_simple_timer(0.0,function() if(self:IsValid()) then self:UpdateTicker() end end)
	self.m_curlRequests = {}
