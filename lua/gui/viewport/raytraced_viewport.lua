...
function gui.RaytracedViewport:OnInitialize()
	...
	self.m_tex = gui.create("render_image", self, 0, 0, self:GetWidth(), self:GetHeight(), 0, 0, 1, 1)
...
function gui.RaytracedViewport:SetTextureFromImageBuffer(imgBuf)
	...
	self.m_tex:SetTexture(
		prosper.create_texture(img, prosper.TextureCreateInfo(), imgViewCreateInfo, samplerCreateInfo)
	)
...
function gui.RaytracedViewport:SetUseElementSizeAsRenderResolution(b)
	self.m_useElementSizeAsRenderResolution = b
end
function gui.RaytracedViewport:GetRenderSettings()
	return self.m_renderSettings
end
function gui.RaytracedViewport:SetRenderSettings(renderSettings)
	self.m_renderSettings = renderSettings
end
function gui.RaytracedViewport:SetGameScene(gameScene)
	self.m_gameScene = gameScene
end
function gui.RaytracedViewport:GetGameScene()
	return self.m_gameScene or game.get_scene()
end
function gui.RaytracedViewport:GetState()
	return self.m_state
end
...
function gui.RaytracedViewport:OnThink()
	...
	if self.m_rtJob:IsCancelled() then
		return
	end
	...
	elseif
		state == pfm.RaytracingRenderJob.STATE_FRAME_COMPLETE
		or state == pfm.RaytracingRenderJob.STATE_SUB_FRAME_COMPLETE
	then
		self.m_rtJob:RenderNextImage()
	end
end
...
function gui.RaytracedViewport:SetToneMapping(toneMapping)
	if toneMapping == self:GetToneMapping() then
		return
	end
	self.m_tex:SetToneMappingAlgorithm(toneMapping)
end
function gui.RaytracedViewport:SetProjectManager(pm)
	self.m_projectManager = pm
end
function gui.RaytracedViewport:SetToneMappingArguments(toneMapArgs)
	self.m_tex:SetToneMappingAlgorithmArgs(toneMapArgs)
end
function gui.RaytracedViewport:GetToneMappingArguments()
	return self.m_tex:GetToneMappingAlgorithmArgs()
end
function gui.RaytracedViewport:GetToneMapping()
	return self.m_tex:GetToneMappingAlgorithm()
end
function gui.RaytracedViewport:GetDOFSettings()
	return self.m_tex:GetDOFSettings()
end
function gui.RaytracedViewport:SetDOFEnabled(b)
	self.m_tex:SetDOFEnabled(b)
end
function gui.RaytracedViewport:SetLuminance(luminance)
	return self.m_tex:SetLuminance(luminance)
end
function gui.RaytracedViewport:GetLuminance()
	return self.m_tex:GetLuminance()
end
function gui.RaytracedViewport:SetExposure(exposure)
	self.m_tex:SetExposure(exposure)
end
function gui.RaytracedViewport:GetExposure()
	return self.m_tex:GetExposure()
end
function gui.RaytracedViewport:GetToneMappedImageElement()
	return self.m_tex
end
function gui.RaytracedViewport:GetSceneTexture()
	return self.m_hdrTex
end
function gui.RaytracedViewport:GetRenderResultRenderSettings()
...
function gui.RaytracedViewport:Refresh(preview, rtJobCallback, startFrame, frameHandler)
	...
	if self.m_projectManager ~= nil then
		...
		frameHandler = function(frameIdx)
			self.m_projectManager:GoToFrame(frameIdx)
		end
	end
	...
	self.m_rtJob:AddCallback("OnFrameStart", function()
		...
		if self.m_rtJob:IsProgressive() == false then
			return
		end
...
function gui.RaytracedViewport:InitializeRenderSettings(settings)
	return settings
end
function gui.RaytracedViewport:OnRenderStart()
	self:CallCallbacks("OnRenderStart")
end
gui.register("raytraced_viewport", gui.RaytracedViewport)
...
function gui.RealtimeRaytracedViewport:OnThink()
	...
	if self.m_tLastUpdate == nil then
		return
	end
	local scene = self:GetGameScene()
	local cam = util.is_valid(scene) and scene:GetActiveCamera() or nil
	...
	if dt < (1.0 / 24.0) then
		return
	end -- Update at roughly 24 FPS
	self.m_tLastUpdate = t

	if self.m_hasDirtyActors ~= true then
		return
	end

	self:FlushDirtyActorChanges()
end
gui.register("realtime_raytraced_viewport", gui.RealtimeRaytracedViewport)
