...
function gui.ParticleExplorer:PopulateContextMenu(pContext,tSelectedFiles)
	...
	elseif(#tSelectedFiles == 0) then
		...
		local isInPtSys = (ext == asset.get_binary_udm_extension(asset.TYPE_PARTICLE_SYSTEM) or ext == asset.get_ascii_udm_extension(asset.TYPE_PARTICLE_SYSTEM))
		if(isInPtSys == false) then
			...
			pContext:AddItem(locale.get_text("pfm_add_particle_collection"),function(pItem)
				...
				te:AddCallback("OnFocusKilled",function()
					...
					util.remove(icon,true)
					util.remove(te,true)
					ptSysName = file.remove_file_extension(ptSysName,asset.get_supported_extensions(asset.TYPE_PARTICLE_SYSTEM))
					if(#ptSysName > 0) then
			...
			pContext:AddItem(locale.get_text("pfm_add_particle_system"),function(pItem)
				...
				te:AddCallback("OnFocusKilled",function()
					...
					if(#ptSysName > 0) then
						...
						if(f == nil) then
							pfm.log("Unable to open file '" .. filePath .. "' for writing!",pfm.LOG_CATEGORY_PFM,pfm.LOG_SEVERITY_WARNING)
							return
						end
						...
						-- in some cases
						time.create_simple_timer(0.5,function() if(self:IsValid()) then self:Refresh() end end)
					end
				end)
			end)
		end
