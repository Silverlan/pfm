...
function Layout:GetFrameContainers()
	return self.m_frameContainers
end
...
Layout.load = function(fileName)
	...
	local function addContainer(udmContainer, parent)
		...
		local c = parent:AddFrameContainer(
			udmContainer:GetValue("name") or ("container" .. tostring(iContainer)),
			defaultRatio
		)
		iContainer = iContainer + 1
		local isHorizontal = udmContainer:GetValue("horizontal", udm.TYPE_BOOLEAN)
		if isHorizontal ~= nil then
			c:SetHorizontal(isHorizontal)
		end
	...
	if util.is_valid(udmRootContainer) then
		addContainer(udmRootContainer, layout)
	end
	return layout
end
...
function FrameContainer:GetFrames()
...
function Element:GetFrameContainerData(identifier)
	return self.m_frameContainers[identifier]
end
...
function Element:RestoreWindowLayoutState(udmData, restoreLayout)
	...
	for containerId, udmContainer in pairs(udmLayout:GetChildren("containers")) do
		...
		for iResizer, ratio in ipairs(frameSizeRatios) do
			...
			if util.is_valid(elResizer) then
				elResizer:SetFraction(ratio)
			end
		end
	end
	...
	for name, udmWindow in pairs(udmLayout:GetChildren("windows")) do
		...
		if udmDetachedWindow:IsValid() then
			...
			if pos ~= nil then
				pos = Vector2i(pos.x * primVideoMode.width, pos.y * primVideoMode.height)
			end
			if size ~= nil then
				size = Vector2i(size.x * primVideoMode.width, size.y * primVideoMode.height)
			end
...
function Element:SaveWindowLayoutState(assetData, saveLayout)
	...
	local frames = {}
	for frame, _ in pairs(frameMap) do
	...
	local function addContainer(containerId, containerInfo)
		...
		for frameId, frameInfo in pairs(container:GetFrames()) do
			if frameInfo.type == "frameContainer" then
				addContainer(frameInfo.name, frameInfo)
			else
			end
		end
	end
