...
function gui.EditableEntry:OnInitialize()
	...
	cbOnChildAdded = self:AddCallback("OnChildAdded", function(el, elChild)
		...
		if elChild:IsType(gui.TYPE_WITEXTENTRY) or elChild:IsType(gui.TYPE_WIDROPDOWNMENU) then
		...
		elseif elChild:IsType(gui.TYPE_WITEXT) then
			elChild:AddCallback("OnTextChanged", function()
				self:UpdateText()
			end)
			elChild:SetVisible(false)
		elseif elChild:IsType(gui.TYPE_PFM_COLOR_ENTRY) then
			...
			update_text_color()
		elseif elChild:IsType(gui.TYPE_TOGGLE_OPTION) then
			self.m_pText:SetVisible(false)
		elseif elChild:IsType(gui.TYPE_PFM_SLIDER) then
...
function gui.EditableEntry:MouseCallback(button, state, mods)
	...
	if util.is_valid(self.m_target) then
		if self.m_target:IsType(gui.TYPE_PFM_SLIDER) then
...
function gui.EditableEntry:StartEditMode(enabled)
	...
	if
		util.is_valid(self.m_target) and (self.m_target:IsType(gui.TYPE_WITEXT) or self.m_target:IsType(gui.TYPE_WIBASE))
	then
		return
	end
	...
	if util.is_valid(self.m_target) then
		if not self.m_target:IsType(gui.TYPE_TOGGLE_OPTION) then
	...
	if value == nil then
		...
		if util.is_valid(self.m_target) then
			if not self.m_target:IsType(gui.TYPE_WIBASE) then
				...
				elseif self.m_target:IsType(gui.TYPE_TOGGLE_OPTION) then
					value = self.m_target:IsChecked() and "1" or "0"
				elseif self.m_target:IsType(gui.TYPE_WITEXT) then
					value = self.m_target:GetText()
				else
...
gui.register("editable_entry", gui.EditableEntry)
