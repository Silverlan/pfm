...
function gui.PFMVideoPlayer:OnInitialize()
	...
	local playControls = gui.create("playback_controls", contents)

	local aspectRatioWrapper = gui.create("aspect_ratio", contents)
...
function gui.PFMVideoPlayer:GetPlayControls()
	return self.m_playbackControls
end
function gui.PFMVideoPlayer:GetVideoPlayerElement()
	return self.m_videoPlayer
end
gui.register("pfm_video_player", gui.PFMVideoPlayer)
