...
include("/shaders/pfm/tonemapping.lua")
...
function gui.PFMPostProcessing:OnInitialize()
	...
	local p = gui.create("pfm_controls_menu", self)
...
function gui.PFMPostProcessing:OnRemove() end
...
function gui.PFMPostProcessing:InitializeDOFControls(p)
	local fApplyDOFSettings = function()
		self:ApplyDOFSettings()
	end
	...
	self.m_ctrlDofUsePentagonBokehShape =
		self:AddToggleControl("pfm_dof_use_pentagon_bokeh_shape", nil, false, fApplyDOFSettings)
	...
	self.m_ctrlPentagonShapeFeather =
		self:AddSliderControl("pfm_dof_pentagon_shape_feather", nil, 0.4, 0, 2, fApplyDOFSettings)
end
function gui.PFMPostProcessing:ApplyDOFSettings()
	...
	flags = math.set_flag(
		flags,
		shader.PFMDepthOfField.DOFSettings.FLAG_BIT_DEBUG_SHOW_FOCUS,
		self.m_ctrlDofShowFocus:IsChecked()
	)
	flags = math.set_flag(
		flags,
		shader.PFMDepthOfField.DOFSettings.FLAG_BIT_DEBUG_SHOW_DEPTH,
		self.m_ctrlDofShowDepth:IsChecked()
	)
	flags = math.set_flag(
		flags,
		shader.PFMDepthOfField.DOFSettings.FLAG_BIT_ENABLE_VIGNETTE,
		self.m_ctrlDofEnableVignette:IsChecked()
	)
	flags = math.set_flag(
		flags,
		shader.PFMDepthOfField.DOFSettings.FLAG_BIT_PENTAGON_BOKEH_SHAPE,
		self.m_ctrlDofUsePentagonBokehShape:IsChecked()
	)
...
gui.register("pfm_post_processing", gui.PFMPostProcessing)
