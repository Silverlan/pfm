...
function gui.PFMSelection:IsInSelection(el)
	...
	return intersect.aabb_with_aabb(
		startPos,
		endPos,
		Vector(el:GetLeft(), el:GetTop(), 0),
		Vector(el:GetRight(), el:GetBottom(), 0)
	) ~= intersect.RESULT_OUTSIDE
end
...
function gui.PFMSelection:UpdateSelectedElements()
	for el, b in pairs(self.m_selectedElements) do
		if
			el:IsValid()
			and self:IsInSelection(el) == false
			and el._impl.mouseSelectionMode ~= gui.PFMSelection.SELECTION_MODE_SELECTED
		then
...
gui.set_mouse_selection_enabled = function(el, enabled)
	...
	if enabled == false then
		if util.is_valid(el._impl.cbMouseSelection) then
			el._impl.cbMouseSelection:Remove()
		end
...
gui.set_mouse_selection_mode = function(el, selectionMode)
	if el._impl == nil or el._impl.mouseSelectable ~= true or el._impl.mouseSelectionMode == selectionMode then
		return
	end
...
gui.set_mouse_selected = function(el, selected)
	if el._impl == nil or el._impl.mouseSelectable ~= true then
		return
	end
	gui.set_mouse_selection_mode(
		el,
		selected and gui.PFMSelection.SELECTION_MODE_SELECTED or gui.PFMSelection.SELECTION_MODE_DESELECTED
	)
end
