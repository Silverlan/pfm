...
function gui.PFMActorEditor:OnInitialize()
	...
	dataVBox = gui.create("vbox", scrollContainer, 0, 0, scrollContainer:GetWidth(), scrollContainer:GetHeight())
	...
	-- self.m_leftRightWeightSlider = gui.create("pfm_weight_slider", self.m_animSetControls)
...
function gui.PFMActorEditor:AddComponentPropertyGroup(actor, udmComponent)
	...
	if util.is_valid(actorGroup) == false then
		local collapsible = gui.create("collapsible_group", self.m_propertyContainer)
...
function gui.PFMActorEditor:AddProperty(name, child, fInitPropertyEl)
	...
	child:AddCallback("OnSelected",function()
		...
		elLabel:AddStyleClass("font_medium")
	...
	elLabel:SetText(name)
	elLabel:AddStyleClass("font_medium")
	elLabel:SetAutoCenterToParentY(true)
	elLabel:SetX(5)
...
function gui.PFMActorEditor:OnControlSelected(actor, actorData, udmComponent, controlData)
	...
	if ctrl ~= nil then
		...
		if metaInfoOpt ~= nil and util.is_valid(container) then
			...
			local curVal = actor:GetMemberValue(optPropName) or false
			elOverlay = gui.create("optional_overlay", ctrl, 0, 0, ctrl:GetWidth(), ctrl:GetHeight(), 0, 0, 1, 1)
...
gui.register("pfm_actor_editor", gui.PFMActorEditor)
