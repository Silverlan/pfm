...
include("/gui/pfm/widgets/texture_slot.lua")
...
function gui.RMAComposerDialog:OnThink()
	...
	if self.m_aoJob:IsComplete() == false then
		return
	end
	...
	if status == util.ParallelJob.JOB_STATUS_SUCCESSFUL then
		...
		console.print_warning("Failed to generate ambient occlusion: ", self.m_aoJob:GetResultMessage())
	end
	self.m_aoJob = nil
end
function gui.RMAComposerDialog:GetRMAMap()
	return self.m_rmaMap
end
...
function gui.RMAComposerDialog:OnInitialize()
	...
	local function add_slot(parent, tex, defaultChannel)
		local vbox = gui.create("vbox", parent, 0, 0, 200, 200)
		vbox:SetAutoFillContentsToWidth(true)
		local texSlot = gui.create("pfm_texture_slot", vbox)
		texSlot:SetSize(200, 200)
		texSlot:SetTexture(tex)
		texSlot:SetClearTexture(tex)

		local elLabel = gui.create("editable_entry", vbox)
		elLabel:SetEmpty()
		...
		channel:Wrap("editable_entry"):SetText(locale.get_text("channel"))
	...
	local contents = gui.create("vbox", el)
	gui.create("WIBase", contents, 0, 0, 1, 30) -- Spacing
	local hbox = gui.create("hbox", contents)
	...
	texSlotAo:AddCallback("PopulateContextMenu", function(texSlotAo, pContext)
		if util.is_valid(self.m_model) and self.m_materialIndex ~= nil then
			pContext:AddItem(locale.get_text("pfm_generate_ambient_occlusion"), function()
				if util.is_valid(self.m_model) == false or pfm.load_unirender() == false then
					return
				end
	...
	workflow:Wrap("editable_entry"):SetText(locale.get_text("pfm_mated_workflow"))
	...
	pButtonOpen:AddCallback("OnPressed", function(pButton)
		...
		if self.m_rmaMap ~= nil then
			self:CallCallbacks("OnRMAComposed", self.m_rmaMap)
		end
		gui.close_dialog()
	end)
...
gui.register("rma_composer_dialog", gui.RMAComposerDialog)
