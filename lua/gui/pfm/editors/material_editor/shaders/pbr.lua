...
function gui.PFMMaterialEditor:InitializePBRControls()
	local mapVbox = gui.create("vbox", self.m_controlBox)
	...
	gui.create("resizer", mapVbox):SetFraction(fractionPerMap)
	...
	gui.create("resizer", mapVbox):SetFraction(fractionPerMap * 2)
	...
	gui.create("resizer", mapVbox):SetFraction(fractionPerMap * 3)
		...
		if matIdx ~= nil then
			pContext:AddItem(locale.get_text("pfm_mated_compose_rma"), function()
				local dialog, frame, fileDialog = gui.create_dialog(function()
					local el = gui.create("rma_composer_dialog")
					if matIdx ~= nil then
	...
	gui.create("resizer", mapVbox):SetFraction(fractionPerMap * 4)
	...
	gui.create("resizer", mapVbox):SetFraction(fractionPerMap * 5)
	...
	--[[local resizer = gui.create("resizer",self.m_controlBox)
	resizer:SetFraction(0.6)
	resizer:SetMoverMode(true)]]

	local ctrlVbox = gui.create("pfm_controls_menu", self.m_controlBox)
	...
	local wrapper = ctrlIOR:Wrap("editable_entry")
...
function gui.PFMMaterialEditor:InitializeShaderMaterialControls()
	...
	local mainCtrlMenu = gui.create("pfm_controls_menu", mapVbox)
	...
	for _, texInfo in ipairs(textures) do
		...
		gui.create("resizer", mapVbox):SetFraction(fraction)
