...
function gui.PFMMaterialEditor:InitializeCyclesOptions(ctrlVbox, shader)
	if shader == "pbr" then
		...
		-- Subsurface method
		local sssControls
		ctrlSSSMethod = ctrlVbox:AddDropDownMenu(
			locale.get_text("pfm_mated_sss_method"),
			"sss_method",
			{
		...
		self:LinkControlToMaterialParameter("method", ctrlSSSMethod, { "subsurface_scattering" }, function(block)
			if block:HasValue("method") == false then
				return
			end
		...
		ctrlSSSFactor = sssControls:AddSliderControl(
			locale.get_text("pfm_mated_sss_factor"),
			"sss_factor",
			0.01,
			0.0,
			0.1,
			function(el, value)
				self:SetMaterialParameter("float", "factor", value, { "subsurface_scattering" })
			end,
			0.001
		)
		...
		-- Subsurface color
		--[[local sssColorEntry = gui.create("pfm_color_entry",sssControls)
		...
		local sssColorEntryWrapper = sssColorEntry:Wrap("editable_entry")
		...
		-- Subsurface radius
		ctrlScatterColor = sssControls:AddColorField(
			locale.get_text("pfm_mated_sss_scatter_color"),
			"sss_scatter_color",
			Color(Vector(0.367, 0.137, 0.068)),
	...
	elseif shader == "toon" then
		...
		local shadeColor = ctrlVbox:AddColorField(
			locale.get_text("pfm_mated_toon_shade_color"),
			"toon_shade_color",
			Color(179, 81, 54),
			function(oldCol, newCol)
				self:SetMaterialParameter(
		...
		self:LinkControlToMaterialParameter("shade_color", shadeColor, propBlock, function(block)
			if block:HasValue("shade_color") == false then
				return
			end
			shadeColor:SetColor(block:GetColor("shade_color"))
		end)

		local specularColor = ctrlVbox:AddColorField(
			locale.get_text("pfm_mated_toon_specular_color"),
			"toon_specular_color",
			Color(255, 255, 255),
			function(oldCol, newCol)
				self:SetMaterialParameter(
		...
		self:LinkControlToMaterialParameter("specular_color", specularColor, propBlock, function(block)
			if block:HasValue("specular_color") == false then
				return
			end
			specularColor:SetColor(block:GetColor("specular_color"))
		end)

		local diffuseSize = ctrlVbox:AddSliderControl(
			locale.get_text("pfm_mated_toon_diffuse_size"),
			"toon_diffuse_size",
			0.9,
			0.0,
			2.0,
			function(el, value)
				self:SetMaterialParameter("float", "diffuse_size", value, propBlock)
			end,
			0.01
		)
		self:LinkControlToMaterialParameter("diffuse_size", diffuseSize, propBlock)

		local diffuseSmooth = ctrlVbox:AddSliderControl(
			locale.get_text("pfm_mated_toon_diffuse_smooth"),
			"toon_diffuse_smooth",
			0.0,
			0.0,
			1.0,
			function(el, value)
				self:SetMaterialParameter("float", "diffuse_smooth", value, propBlock)
			end,
			0.01
		)
		self:LinkControlToMaterialParameter("diffuse_smooth", diffuseSmooth, propBlock)

		local specularSize = ctrlVbox:AddSliderControl(
			locale.get_text("pfm_mated_toon_specular_size"),
			"toon_specular_size",
			0.2,
			0.0,
			2.0,
			function(el, value)
				self:SetMaterialParameter("float", "specular_size", value, propBlock)
			end,
			0.01
		)
		self:LinkControlToMaterialParameter("specular_size", specularSize, propBlock)

		local specularSmooth = ctrlVbox:AddSliderControl(
			locale.get_text("pfm_mated_toon_specular_smooth"),
			"toon_specular_smooth",
			0.0,
			0.0,
			1.0,
			function(el, value)
				self:SetMaterialParameter("float", "specular_smooth", value, propBlock)
			end,
			0.01
		)
