...
function gui.PFMTimelineGraph:OnInitialize()
	...
	self.m_boneList = gui.create("WITreeList",self.m_scrollContainer,0,0,self.m_scrollContainer:GetWidth(),self.m_scrollContainer:GetHeight(),0,0,1,0)
...
function gui.PFMTimelineGraph:SetTimeRange(startTime,endTime)
...
function gui.PFMTimelineGraph:Setup(actor,channelClip)
	...
	for _,bone in ipairs(mdlC:GetBoneList():GetTable()) do
		...
		if(bonePosChannel ~= nil) then
			...
			if(layer ~= nil) then
				self:AddBoneTransform(layer,itemBone,"Position X",Color.Red,function(v) return v.x end)
				--Color(227,90,90)
				self:AddBoneTransform(layer,itemBone,"Position Y",Color.Lime,function(v) return v.y end)
				--Color(84,168,70)
				self:AddBoneTransform(layer,itemBone,"Position Z",Color.Blue,function(v) return v.z end)
				--Color(96,127,193)
			end
		end
		if(boneRotChannel ~= nil) then
			...
			if(layer ~= nil) then
				self:AddBoneTransform(layer,itemBone,"Rotation X",Color.Red,function(v) return v:ToEulerAngles().p end)
				self:AddBoneTransform(layer,itemBone,"Rotation Y",Color.Lime,function(v) return v:ToEulerAngles().y end)
				self:AddBoneTransform(layer,itemBone,"Rotation Z",Color.Blue,function(v) return v:ToEulerAngles().r end)
			end
		end
	end
