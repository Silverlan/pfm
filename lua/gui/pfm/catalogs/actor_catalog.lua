...
function gui.PFMActorCatalog:OnInitialize()
	...
	local entGhost
	local explorer = gui.create("icon_grid_view", scrollContainer, 0, 0, self:GetWidth(), self:GetHeight())
		...
		gui.enable_drag_and_drop(icon, "ModelCatalog", function(elGhost)
			...
			elGhost:AddCallback("OnDragTargetHoverStart", function(elGhost, elTgt)
				...
				if ghostC ~= nil then
					...
					if elTgt:IsType(gui.TYPE_VIEWPORT) then
...
gui.register("pfm_actor_catalog", gui.PFMActorCatalog)
