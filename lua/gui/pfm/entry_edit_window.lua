...
function PfmEditEntryWindow:OnInitialize()
	...
	btOk:AddCallback("OnMousePressed",function()
		if(self:CallCallbacks("OnOk") == util.EVENT_REPLY_HANDLED) then return end
	...
	btCancel:AddCallback("OnMousePressed",function()
		if(self:CallCallbacks("OnCancel") == util.EVENT_REPLY_HANDLED) then return end
		self:GetFrame():Remove()
	end)
...
pfm.open_entry_edit_window = function(title,cb)
	...
	p:SetTitle(title)
	if(cb ~= nil) then
		p:AddCallback("OnOk",function() return cb(true) end)
		p:AddCallback("OnCancel",function() return cb(false) end)
	end
	return p
end
