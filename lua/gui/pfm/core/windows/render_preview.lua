...
function gui.PFMRenderPreviewWindow:__init(parent)
	...
	if elTitle ~= nil then
		btRefresh:SetLeft(elTitle:GetRight() + 5)
	end
...
function gui.PFMRenderPreviewWindow:OnThink()
	...
	if self.m_raytracingJob:IsSuccessful() then
		...
		local tex =
			prosper.create_texture(img, prosper.TextureCreateInfo(), imgViewCreateInfo, prosper.SamplerCreateInfo())
		tex:SetDebugName("render_preview_window_tex")

		if util.is_valid(self.m_preview) then
			self.m_preview:SetTexture(tex)
		end
		if util.is_valid(self.m_raytracingProgressBar) then
...
function gui.PFMRenderPreviewWindow:GetFrame()
	return self.m_previewFrame
end
function gui.PFMRenderPreviewWindow:Remove()
	if self.m_raytracingJob ~= nil then
		self.m_raytracingJob:Cancel()
	end
	if util.is_valid(self.m_previewFrame) then
		self.m_previewFrame:Remove()
	end
end
