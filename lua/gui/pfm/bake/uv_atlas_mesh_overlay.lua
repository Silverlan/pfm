...
function gui.UvAtlasMeshOverlay:SetLightmapCache(lmCache) self.m_lightmapCache = lmCache end
function gui.UvAtlasMeshOverlay:SetEntity(ent) self.m_entity = ent end
...
function gui.UvAtlasMeshOverlay:SetPixelColor(x,y,color)
	if(x >= self.m_imgBuffer:GetWidth() or y >= self.m_imgBuffer:GetHeight()) then return end
...
function gui.UvAtlasMeshOverlay:OnUpdate()
	if(util.is_valid(self.m_entity) == false or self.m_lightmapCache == nil) then return end
			...
			for _,sm in ipairs(m:GetSubMeshes()) do
				...
				if(lightmapUvs ~= nil) then
					...
					for i=1,#indices,3 do
						...
						self:DrawLine(Vector(1,0,0),uv0.x *imgBuf:GetWidth(),uv0.y *imgBuf:GetHeight(),Vector(0,1,0),uv1.x *imgBuf:GetWidth(),uv1.y *imgBuf:GetHeight())
						self:DrawLine(Vector(1,0,0),uv1.x *imgBuf:GetWidth(),uv1.y *imgBuf:GetHeight(),Vector(0,1,0),uv2.x *imgBuf:GetWidth(),uv2.y *imgBuf:GetHeight())
						self:DrawLine(Vector(1,0,0),uv2.x *imgBuf:GetWidth(),uv2.y *imgBuf:GetHeight(),Vector(0,1,0),uv0.x *imgBuf:GetWidth(),uv0.y *imgBuf:GetHeight())
					end
				end
			end
		end
