...
gui.register("pfm_tree_view", gui.PFMTreeView)
...
function gui.PFMTreeViewElement:OnInitialize()
	...
	self.m_header = gui.create("hbox", self.m_vBox, 0, 0, self:GetWidth(), self:GetHeight())
...
function gui.PFMTreeViewElement:InitializeChildBox()
	...
	self.m_childHBox = gui.create("hbox", self.m_vBox, 0, 0, self.m_vBox:GetWidth(), self.m_vBox:GetHeight())
...
function gui.PFMTreeViewElement:AddItem(text, fPopulate, insertIndex, identifier)
	self:InitializeChildBox()
	local item = gui.create("pfm_tree_view_element")
	item:SetText(text)
...
gui.register("pfm_tree_view_element", gui.PFMTreeViewElement)
...
gui.register("pfm_tree_expand_icon", gui.PFMTreeExpandIcon)
