...
function util.ImagePostProcessor:Remove() end
function util.ImagePostProcessor:InitializeSceneTexture(w, h)
	if self.m_hdrTex ~= nil and self.m_hdrTex:GetWidth() == w and self.m_hdrTex:GetHeight() == h then
	...
	local texHdr =
		prosper.create_texture(imgHdr, prosper.TextureCreateInfo(), prosper.ImageViewCreateInfo(), samplerCreateInfo)
	...
	self.m_hdrRt = prosper.create_render_target(
		prosper.RenderTargetCreateInfo(),
		{ self.m_hdrTex },
		shaderComposition:GetRenderPass()
	)
...
function util.ImagePostProcessor:InitializeDepthTexture(w, h, nearZ, farZ)
	...
	if self.m_depthTex ~= nil and self.m_depthTex:GetWidth() == w and self.m_depthTex:GetHeight() == h then
		return
	end
	self.m_depthTex = nil
	collectgarbage() -- Make sure the old texture is cleared from cache
	...
	imgCreateInfo.usageFlags =
		bit.bor(prosper.IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, prosper.IMAGE_USAGE_SAMPLED_BIT)
	imgCreateInfo.tiling = prosper.IMAGE_TILING_OPTIMAL
	imgCreateInfo.memoryFeatures = prosper.MEMORY_FEATURE_GPU_BULK_BIT
	imgCreateInfo.postCreateLayout = prosper.IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
	...
	samplerCreateInfo.addressModeW = prosper.SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE
	local texDepth =
		prosper.create_texture(imgDepth, prosper.TextureCreateInfo(), prosper.ImageViewCreateInfo(), samplerCreateInfo)
...
function util.ImagePostProcessor:UpdateGameSceneTextures()
	...
	if renderer == nil or cam == nil then
		return
	end
	...
	drawCmd:RecordImageBarrier(
		depthTex:GetImage(),
		prosper.PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
		...
		bit.bor(prosper.ACCESS_MEMORY_READ_BIT, prosper.ACCESS_MEMORY_WRITE_BIT),
...
function util.ImagePostProcessor:RenderParticleSystemDepth(drawCmd)
	...
	drawCmd:RecordImageBarrier(
		...
		prosper.PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
		prosper.PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT,
		prosper.IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
		prosper.IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
		bit.bor(prosper.ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT, prosper.ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT),
		bit.bor(prosper.ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT, prosper.ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT)
	)
	local rtDepth = prosper.create_render_target(
		prosper.RenderTargetCreateInfo(),
		{ self.m_depthTex },
		shader.BaseParticle2D.get_depth_pipeline_render_pass()
	)
