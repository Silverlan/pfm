...
function gui.Axis:OnRemove()
	if util.is_valid(self.m_cbOnPropertiesChanged) then
		self.m_cbOnPropertiesChanged:Remove()
	end
...
function gui.Axis:GetAxis()
	return self.m_axis
end
function gui.Axis:IsHorizontal()
	return self.m_horizontal
end
function gui.Axis:IsVertical()
...
function gui.Axis:AttachElementToValueWithUdmProperty(el, udmEl, propName, translateTime)
	...
	table.insert(
		self.m_items[i].callbacks,
		udmEl:AddChangeListener(propName, function(c, newTime)
			self:UpdateAttachment(i, (translateTime ~= nil) and translateTime(newTime) or newTime)
		end)
	)
end
...
function gui.Axis:UpdateItems()
	...
	for i, itemData in pairs(self.m_items) do
		if itemData.element:IsValid() then
			...
			table.insert(invalidItems, i)
		end
	end

	for _, i in ipairs(invalidItems) do
		self.m_items[i] = nil
	end
end
