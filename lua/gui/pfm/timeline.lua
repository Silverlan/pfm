...
function gui.PFMTimeline:OnTimelineUpdate()
	...
	if(util.is_valid(self.m_timelineGraph)) then
		...
		self.m_timelineGraph:UpdateAxisRanges(timeAxis:GetStartOffset(),timeAxis:GetZoomLevel(),dataAxis:GetStartOffset(),dataAxis:GetZoomLevel())
	end
end
...
function gui.PFMTimeline:AddBookmarkSet(bms,timeFrame) return self.m_timeline:AddBookmarkSet(bms,timeFrame) end
...
function gui.PFMTimeline:GetControlButton(identifier) return self.m_controlButtons[identifier] end
	...
	if(args[1] == "select") then
		...
		if(bt ~= nil) then
			...
			time.create_simple_timer(0.1,function()
				if(bt:IsValid()) then bt:InjectMouseInput(bt:GetCursorPos(),input.MOUSE_BUTTON_LEFT,input.STATE_RELEASE,input.MOD_NONE) end
			end)
		end
	end
end)
