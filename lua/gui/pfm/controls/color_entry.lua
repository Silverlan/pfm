...
function gui.PFMColorEntry:OnRemove()
	if util.is_valid(self.m_colorSelectorWrapper) then
		self.m_colorSelectorWrapper:Remove()
	end
end
function gui.PFMColorEntry:OpenColorSelector()
	...
	local el = gui.create("pfm_color_selector", elWrapper)
	...
	el:AddCallback("OnUserInputStarted", function()
		self:CallCallbacks("OnUserInputStarted")
	end)
	el:AddCallback("OnUserInputEnded", function()
...
gui.register("pfm_color_entry", gui.PFMColorEntry)
