...
include("wheel.lua")
include("/gui/pfm/controls/brightness_slider.lua")
...
function gui.PFMColorSelector:OnInitialize()
	...
	self.m_coreContents = gui.create("vbox", self.m_contents)
	self.m_coreContents:SetAutoFillContentsToWidth(true)
	gui.create("WIBase", self.m_coreContents, 0, 0, 1, 10) -- Gap

	local selectorContents = gui.create("hbox", self.m_coreContents)
	self.m_colorWheel = gui.create("pfm_color_wheel", selectorContents)
	self.m_colorWheel:AddCallback("OnUserInputStarted", function()
		self:CallCallbacks("OnUserInputStarted")
	end)
	self.m_colorWheel:AddCallback("OnUserInputEnded", function()
		self:CallCallbacks("OnUserInputEnded")
	end)
	self.m_colorWheel:AddCallback("OnColorChanged", function(el, col)
		if self.m_skipCallbacks == true then
			return
		end
	...
	local brightnessSlider = gui.create("pfm_brightness_slider", selectorContents)
	...
	brightnessSlider:AddCallback("OnUserInputStarted", function()
		self:CallCallbacks("OnUserInputStarted")
	end)
	brightnessSlider:AddCallback("OnUserInputEnded", function()
		self:CallCallbacks("OnUserInputEnded")
	end)
	...
	colorMode:Wrap("editable_entry"):SetText(locale.get_text("color_mode"))
	...
	for _, colData in ipairs(presetColorTable) do
		...
		if el ~= nil then
			...
			gui.create("WIOutlinedRect", elCol, 0, 0, elCol:GetWidth(), elCol:GetHeight(), 0, 0, 1, 1)
	...
	preset:AddCallback("OnOptionSelected", function(el, option)
		--print("OnOptionSelected: ", option, debug.traceback())
		self.m_origColor = nil
		if self.m_skipPresetCallback ~= true then
	...
	--preset:Wrap("editable_entry"):SetText(locale.get_text("preset"))

	local function create_slider(text, min, max, fApply)
		local slider = gui.create("pfm_slider", self.m_coreContents)
		...
		slider:AddCallback("OnUserInputStarted", function()
			self:CallCallbacks("OnUserInputStarted")
		end)
		slider:AddCallback("OnUserInputEnded", function()
			self:CallCallbacks("OnUserInputEnded")
		end)
	...
	local teHexWrapper = teHex:Wrap("editable_entry")
...
function gui.PFMColorSelector:GetColorWheel()
	return self.m_colorWheel
end
function gui.PFMColorSelector:GetSelectedColorRGB()
	return self.m_colorRGB
end
function gui.PFMColorSelector:GetSelectedColorHSV()
	return self.m_colorHSV
end
function gui.PFMColorSelector:GetSelectedColorHex()
	return self.m_colorRGB:ToHexColorRGB()
end
...
function gui.PFMColorSelector:GetBrightness()
	return self.m_colorHSV.v
end
function gui.PFMColorSelector:UpdateColor(color)
	...
	local bestMatch
	local bestMatchDist
	for i, cdata in ipairs(presetColorTable) do
		local dist = self.m_colorHSV:Distance(cdata[3])
		if bestMatch == nil or dist < bestMatchDist then
			bestMatch = i - 1
			bestMatchDist = dist
		end
	end
	if bestMatch ~= nil then
		self.m_skipPresetCallback = true
		self.m_preset:SelectOption(bestMatch)
		self.m_skipPresetCallback = nil
	end

	self:CallCallbacks("OnColorChanged")
end
function gui.PFMColorSelector:GetSelectedColor()
	return self.m_color
end
local presetColors = {
	-- Pink Colors
...
for i, preset in ipairs(presetColors) do
	preset[3] = preset[1]:ToHSVColor()
end
function gui.PFMColorSelector:GetPresetColors()
	return presetColors
end
gui.register("pfm_color_selector", gui.PFMColorSelector)
