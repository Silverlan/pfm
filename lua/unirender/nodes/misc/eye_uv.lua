...
local function dot4(v0,v1) return v0[1] *v1[1] +v0[2] *v1[2] +v0[3] *v1[3] +v0[4] *v1[4] end
unirender.NODE_EYE_UV = unirender.register_node("eye_uv",function(desc)
	local inProjUXyz = desc:RegisterInput(unirender.Socket.TYPE_VECTOR,unirender.Node.eye_uv.IN_IRIS_PROJ_U_XYZ,Vector())
	local inProjUW = desc:RegisterInput(unirender.Socket.TYPE_FLOAT,unirender.Node.eye_uv.IN_IRIS_PROJ_U_W,0.0)
	local inProjVXyz = desc:RegisterInput(unirender.Socket.TYPE_VECTOR,unirender.Node.eye_uv.IN_IRIS_PROJ_V_XYZ,Vector())
	...
	local inIrisMaxDilationFactor = desc:RegisterInput(unirender.Socket.TYPE_FLOAT,unirender.Node.eye_uv.IN_IRIS_MAX_DILATION_FACTOR,1.0)
	...
	local geo = desc:AddNode(unirender.NODE_GEOMETRY):GetOutputSocket(unirender.Node.geometry.OUT_POSITION) *util.metres_to_units(1.0)
	...
	local nodeUvX = dot4(
		{geo.x,geo.y,geo.z,1.0},
		{inProjUXyz.x,inProjUXyz.y,inProjUXyz.z,inProjUW}
	)
	local nodeUvY = dot4(
		{geo.x,geo.y,geo.z,1.0},
		{inProjVXyz.x,inProjVXyz.y,inProjVXyz.z,inProjVW}
	)
	...
	local factor = desc:AddConstantNode(1.0):Lerp(pupilCenterToBorder,inIrisDilation:Clamp(0.0,inIrisMaxDilationFactor) *2.5 -1.25)
