...
unirender.NODE_EMBER_MATERIAL = unirender.register_node("ember_material", function(desc)
	local inColor =
		desc:RegisterInput(unirender.Socket.TYPE_COLOR, unirender.Node.ember_material.IN_COLOR, Vector(0, 0, 0))
	...
	local mix =
		principled:GetPrimaryOutputSocket():Mix(emission:GetPrimaryOutputSocket(), colorRamp:GetPrimaryOutputSocket())
	mix:Link(outShader)
end)
