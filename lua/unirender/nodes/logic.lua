...
local function run_operator(self, op, sockets, addSockets)
	...
	for i, socket in ipairs(sockets) do
		...
		if socket:IsConcreteValue() == false and socket:IsOutputSocket() == false then
			node = socket:GetNode()
		end -- Special case: This is an input socket of a group node
		if node == nil then
			node = socket:GetNode()
			if node ~= nil then
				node = node:GetParent()
	...
	if self:IsConcreteValue() == false and self:IsOutputSocket() == false then
		node = self:GetNode()
	end -- Special case: This is an input socket of a group node
	if node == nil then
		error("Currently not implemented for concrete types!")
	end
...
function unirender.Socket:Not()
	return run_operator(self, unirender.NODE_NOT)
end
...
function unirender.Socket:Or(socket)
	return run_operator(self, unirender.NODE_OR, { socket })
end
...
function unirender.Socket:And(socket)
	return run_operator(self, unirender.NODE_AND, { socket })
end
...
function unirender.Socket:Nor(socket)
	return run_operator(self, unirender.NODE_NOR, { socket })
end
...
function unirender.Socket:Nand(socket)
	return run_operator(self, unirender.NODE_NAND, { socket })
end
...
function unirender.Socket:Xor(socket)
	return run_operator(self, unirender.NODE_XOR, { socket })
end
...
function unirender.Socket:IsEqualTo(socket, tolerance)
	return run_operator(self, unirender.NODE_EQUAL, { socket }, { [unirender.Node.equal.IN_TOLERANCE] = tolerance })
end
