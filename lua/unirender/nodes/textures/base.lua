...
unirender.NODE_ALBEDO_TEXTURE = unirender.register_node("albedo_texture", function(desc)
	local inTexture = desc:RegisterProperty(
		...
		unirender.Node.albedo_texture.IN_TEXTURE,
		unirender.get_texture_path("error")
	)
	local inAlphaMode = desc:RegisterInput(
		unirender.Socket.TYPE_ENUM,
		unirender.Node.albedo_texture.IN_ALPHA_MODE,
		game.Material.ALPHA_MODE_OPAQUE
	)
	local inAlphaCutoff =
		desc:RegisterInput(unirender.Socket.TYPE_FLOAT, unirender.Node.albedo_texture.IN_ALPHA_CUTOFF, 0.5)
	local inColorFactor =
		desc:RegisterInput(unirender.Socket.TYPE_COLOR, unirender.Node.albedo_texture.IN_COLOR_FACTOR, Vector(1, 1, 1))
	local inAlphaFactor =
		desc:RegisterInput(unirender.Socket.TYPE_FLOAT, unirender.Node.albedo_texture.IN_ALPHA_FACTOR, 1.0)
...
unirender.NODE_EMISSION_TEXTURE = unirender.register_node("emission_texture", function(desc)
	local inTexture = desc:RegisterProperty(
		unirender.Socket.TYPE_STRING,
		unirender.Node.emission_texture.IN_TEXTURE,
		unirender.get_texture_path("error")
	)
	local inUseAlphaChannel =
		desc:RegisterInput(unirender.Socket.TYPE_BOOL, unirender.Node.emission_texture.IN_USE_ALPHA_CHANNEL, false)
	local inColorFactor = desc:RegisterInput(
		unirender.Socket.TYPE_COLOR,
		unirender.Node.emission_texture.IN_COLOR_FACTOR,
		Vector(1, 1, 1)
	)
