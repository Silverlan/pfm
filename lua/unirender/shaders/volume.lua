...
function unirender.VolumeShader:InitializeCombinedPass(desc,outputNode)
	...
	if(cyclesRenderer) then
		...
		if(volData == nil) then return end
		...
		vol:SetProperty(unirender.Node.principled_volume.IN_ABSORPTION_COLOR,volData:GetVector("absorption_color",Vector(0.0,0.0,0.0)))
		vol:SetProperty(unirender.Node.principled_volume.IN_EMISSION_STRENGTH,volData:GetFloat("emission_strength",0.0))
		vol:SetProperty(unirender.Node.principled_volume.IN_EMISSION_COLOR,volData:GetVector("emission_color",Vector(0.0,0.0,0.0)))
		vol:SetProperty(unirender.Node.principled_volume.IN_BLACKBODY_INTENSITY,volData:GetFloat("blackbody_intensity",0.0))
		vol:SetProperty(unirender.Node.principled_volume.IN_BLACKBODY_TINT,volData:GetVector("blackbody_tint",Vector(0.0,0.0,0.0)))
