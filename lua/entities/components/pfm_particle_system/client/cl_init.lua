...
function ents.PFMParticleSystem:OnTick(dt)
	...
	if self.m_queuedSimOffset == 0 then
		return
	end
...
function ents.PFMParticleSystem:OnOffsetChanged(offset)
	local ptC = self:GetEntity():GetComponent(ents.COMPONENT_PARTICLE_SYSTEM)
	if ptC:IsActive() == false then
		return
	end
	if self:IsSimulating() == false and (ptC == nil or ptC:IsDying() == false) then
		return
	end
...
function ents.PFMParticleSystem:GetParticleData()
	return self.m_particleData
end
...
function ents.PFMParticleSystem:InitializeParticleSystem()
	...
	if ptSystemName == nil then
		_ptTest = particleData
	end
	if numParticles == 0 and #ptSystemName == 0 then
		return
	end
	...
	if #ptSystemName > 0 then
		...
		game.precache_particle_system(
			"sessions/" .. pfm.get_project_manager():GetProject():GetName() .. "_instanced",
			true
		)
	...
	if #ptSystemName == 0 then
		...
		for _, initializerData in ipairs(def:GetInitializers():GetTable()) do
			...
			if string.compare(name, "position within sphere random", false) then
				local speedInLocalCoordinateSystemMin =
					convert_vector(initializerData:GetProperty("speed_in_local_coordinate_system_min"):GetValue())
				local speedInLocalCoordinateSystemMax =
					convert_vector(initializerData:GetProperty("speed_in_local_coordinate_system_max"):GetValue())
	...
	for i, cp in ipairs(particleData:GetControlPoints():GetTable()) do
		table.insert(
			self.m_listeners,
			cp:GetPositionAttr():AddChangeListener(function(newPos)
				if ptC == nil then
					return
				end
		...
		table.insert(
			self.m_listeners,
			cp:GetRotationAttr():AddChangeListener(function(newRot)
				local ptC = self:GetEntity():GetComponent(ents.COMPONENT_PARTICLE_SYSTEM)
				if ptC == nil then
					return
				end
	...
	table.insert(
		self.m_listeners,
		particleData:GetSimulatingAttr():AddChangeListener(function(simulating)
			self:UpdateSimulationState()
		end)
	)
	table.insert(
		self.m_listeners,
		particleData:GetEmittingAttr():AddChangeListener(function(emitting)
			self:UpdateSimulationState()
		end)
	)
	self:UpdateSimulationState()
...
function ents.PFMParticleSystem:ApplySimulationState()
	...
	if ptC ~= nil then
		if self:IsSimulating() == false then
			if ptC:IsActive() and ptC:IsDying() == false then
				ptC:Die()
			end
...
function ents.PFMParticleSystem:IsSimulating()
	return self.m_simulating
end
