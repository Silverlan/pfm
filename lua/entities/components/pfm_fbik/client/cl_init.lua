	...
	if useModelBones == true then
		if true then
			...
			local function add_bone(bone)
				local ikBone =
					solverC:AddBone(bone:GetID(), get_bone_pos(bone:GetID()), get_bone_rot(bone:GetID()), 1, 1)
			...
			local function add_hinge_constaint(parentName, name, min, max)
				solverC:AddHingeConstraint(
					skeleton:LookupBone(parentName),
					skeleton:LookupBone(name),
					min * limitScale,
					max * limitScale
				)
			...
			debug.draw_line(p:GetOrigin(), p:GetOrigin() + p:GetForward() * l, dbgInfo)
			dbgInfo:SetColor(Color.Green)
			debug.draw_line(p:GetOrigin(), p:GetOrigin() + p:GetRight() * l, dbgInfo)
			dbgInfo:SetColor(Color.Blue)
			debug.draw_line(p:GetOrigin(), p:GetOrigin() + p:GetUp() * l, dbgInfo)
			...
			local Bind_LeftShoulder = solverC:AddBone(
				Bind_LeftShoulderId,
				get_bone_pos(Bind_LeftShoulderId),
				get_bone_rot(Bind_LeftShoulderId),
				1,
				1
			)
			local Bind_LeftArm = solverC:AddBone(
				Bind_LeftArmId,
				get_bone_pos(Bind_LeftArmId),
				get_bone_rot(Bind_LeftArmId),
				1,
				1,
				Bind_LeftShoulder
			)
			local Bind_LeftForeArm = solverC:AddBone(
				Bind_LeftForeArmId,
				get_bone_pos(Bind_LeftForeArmId),
				get_bone_rot(Bind_LeftForeArmId),
				1,
				1,
				Bind_LeftArm
			)
			local Bind_LeftHand = solverC:AddBone(
				Bind_LeftHandId,
				get_bone_pos(Bind_LeftHandId),
				get_bone_rot(Bind_LeftHandId),
				1,
				1,
				Bind_LeftForeArm
			)
		...
		solver
			:AddSwingLimit(baseBone, upperArm, axisOffset:GetUp(), subRot:GetUp(), math.rad(swingRange))
			:SetRigidity(1000)
		-- TODO: Twist Limit
