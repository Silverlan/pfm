...
function ents.PFMClip:SetCamPos(pos) self.m_camPos = pos end
function ents.PFMClip:SetCamRot(rot) self.m_camRot = rot end
function ents.PFMClip:GetSoundEvents() return self.m_soundEvents end
function ents.PFMClip:AddSoundEvent(ev)
...
function ents.PFMClip:AddActor(name,mdlName,origin,rot)
...
function ents.PFMClip:GetActor(name)
	for _,actor in ipairs(self.m_actors) do
		if(actor:GetComponent(ents.COMPONENT_NAME):GetName() == name) then return actor end
	end
end
function ents.PFMClip:AddSoundEvent(name,start,duration,volume,pitch,origin,direction)
	-- TODO
end
function ents.PFMClip:AddCamera(name,origin,rot)
...
function ents.PFMClip:GetActors() return self.m_actors end
function ents.PFMClip:SetOffsetTransform(pos,rot)
	self.m_offsetTransform = {pos,rot}
end
function ents.PFMClip:SetCameraEnabled(b) self.m_bCameraEnabled = b end
ents.COMPONENT_PFM_TRACK = ents.register_component("pfm_track",ents.PFMClip)
