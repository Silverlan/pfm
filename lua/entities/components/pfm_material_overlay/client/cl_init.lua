...
function ents.PFMMaterialOverlay:OnRemove()
	if util.is_valid(self.m_guiEl) then
		self.m_guiEl:Remove()
	end
...
function ents.PFMMaterialOverlay:GetMaterialOverlayData()
	return self.m_materialOverlayData
end
...
function ents.PFMMaterialOverlay:Setup(overlayClipC, matOverlayData, fadeInTime, fadeOutTime)
	...
	self.m_cbOnOffsetChanged = overlayClipC:AddEventCallback(
		ents.PFMFilmClip.EVENT_ON_OFFSET_CHANGED,
		function(filmClipOffset, absOffset)
...
function ents.PFMMaterialOverlay:OnOffsetChanged(offset)
	if util.is_valid(self.m_guiEl) == false then
		return
	end
...
function ents.PFMMaterialOverlay:OnEntitySpawn()
	...
	if self.m_fadeInTime > 0.0 or self.m_fadeOutTime > 0.0 then
		el:SetAlpha(0.0)
	end
	self.m_guiEl = el
end
...
ents.register_component(
	"pfm_material_overlay",
	ents.PFMMaterialOverlay,
	"pfm",
