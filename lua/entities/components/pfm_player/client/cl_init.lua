...
function ents.PFMPlayer:IsTaunting()
	return self.m_taunting
end

function ents.PFMPlayer:ClearTaunt()
	self.m_taunting = false
	if util.is_valid(self.m_cbHandleActionInput) then
		self.m_cbHandleActionInput:Remove()
	end
	if util.is_valid(self.m_cbOnAnimationComplete) then
		self.m_cbOnAnimationComplete:Remove()
	end
end
