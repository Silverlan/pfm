...
function Component:UpdateGizmo()
	...
	if type == Component.TYPE_TRANSLATION then
		...
		if localToggle then
			pose:SetRotation(self.m_transformComponent:GetEntity():GetRotation())
		elseif space == ents.UtilTransformComponent.SPACE_VIEW then
			local cam = ents.ClickComponent.get_camera()
...
function Component:StartTransform(hitPos)
	...
	gizmo:SetInteractionStart(
		...
		math.ScaledTransform(
			self.m_transformComponent:GetEntity():GetPos(),
			targetSpaceRotation:GetInverse() * self.m_transformComponent:GetEntity():GetRotation(),
