...
function Component:GetRelativePose()
	return self.m_relPose
end
...
function Component:Impersonate(ent)
	...
	if impersonateeC == self.m_impersonateeC then
		return
	end
	...
	if util.is_valid(self.m_entImpostor) then
		return self.m_entImpostor
	end
	...
	if renderCImpersonatee ~= nil and renderCThis ~= nil then
		renderCThis:SetCastShadows(renderCImpersonatee:ShouldCastShadows())
	end
	...
	for _, c in ipairs(charComponents) do
		if entImpersonatee:HasComponent(c) then
			entThis:AddComponent(c)
		end
	end
...
function Component:GetImpersonatee()
	return self.m_impersonateeC
end

function Component:OnAnimationReset()
	self:UpdateModel(self:GetEntity():GetModel())
end

function Component:UpdateModel(mdl)
	local impersonateeC = self:GetImpersonatee()
	if util.is_valid(impersonateeC) == false then
		return
	end

	if mdl == impersonateeC:GetEntity():GetModel() then
		mdl = nil
	end

	local renderModeImpostor = (mdl ~= nil) and game.SCENE_RENDER_PASS_WORLD or game.SCENE_RENDER_PASS_NONE
	local renderModeImpostee = (renderModeImpostor == game.SCENE_RENDER_PASS_WORLD) and game.SCENE_RENDER_PASS_NONE
		or game.SCENE_RENDER_PASS_WORLD
	...
	if retargetC ~= nil then
		if shouldRig then
			retargetC:RigToActor(entImpersonatee)
		else
			retargetC:Unrig()
		end
	end
	...
	if headhackC ~= nil then
		...
		if util.is_valid(headhackHead) then
			headTarget = headhackHead
		end
	end
	local retargetMorphC = ent:AddComponent("retarget_morph")
	if retargetMorphC ~= nil then
		if shouldRig then
			retargetMorphC:RigToActor(headTarget)
		else
			retargetMorphC:Unrig()
		end
	end
end
