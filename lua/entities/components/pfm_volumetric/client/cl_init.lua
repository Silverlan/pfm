...
local function update_material(c) c:UpdateMaterial() end
...
function Component:OnBoundsChanged(min,max)
	if(self.m_cubeModel == false) then return end
...
function Component:OnModelChanged()
	...
	if(self.m_cubeModel == false) then self:GetEntity():SetScale(Vector(1,1,1)) end
	self:UpdateMaterial()
end
...
function Component:UpdateMaterial()
	...
	if(self.m_usingCustomMaterial == true) then return end
