...
function ents.RetargetRig.BoneRemapper:__init(
	skeletonSrc,
	refSrc,
	skeletonDst,
	refDst,
	...
	for boneId0, data0 in pairs(boneData0) do
		...
		if pose0 ~= nil then
			...
			for boneId1, data1 in pairs(boneData1) do
				local sim =
					string.calc_levenshtein_similarity(data0.bone:GetName():lower(), data1.bone:GetName():lower())
			...
			for _, data in ipairs(data0.targetBones) do
				...
				if ents.RetargetRig.Rig.is_bone_relation_in_cache(data0.bone:GetName(), data.bone:GetName()) then
					sim = 1.0
				end
			...
			table.sort(data0.targetBones, function(a, b)
...
function ents.RetargetRig.BoneRemapper:AutoRemap()
	...
	local function check_hierarchy_match(bone)
		...
		if n > 0 then
			...
			table.sort(boneData0[bone:GetID()].targetBones, function(a, b)
				return a.weight > b.weight
			end)
		end
...
foreach bone a -> grab most likely candidate b
	...
	table.sort(probabilityTable, function(a, b)
		return a[3] > b[3]
	end)
