...
local function find_common_substrings(names)
	...
	for sub,_ in pairs(commonSubstrings) do table.insert(sortedCommonSubstrings,sub) end
	table.sort(sortedCommonSubstrings,function(a,b) return commonSubstrings[a] > commonSubstrings[b] end)
...
local function strip_common_generic_substrings(names,threshold)
	...
	if(sub ~= nil) then
		...
		while(i <= #sortedCommonSubstrings) do
			if(#sortedCommonSubstrings[i] >= #sub or sub:find(sortedCommonSubstrings[i]) == nil or get_average_makeup_percentage(sortedCommonSubstrings[i],names) < 0.2) then
				table.remove(sortedCommonSubstrings,i)
			else
				i = i +1
			end
		end
		table.sort(sortedCommonSubstrings,function(a,b) return #a > #b end) -- We need to make sure to remove the longer matches first
	end
...
function ents.RetargetRig.autoretarget(names0,names1,threshold,origNames0,origNames1)
	...
	for i,name0 in ipairs(names0) do
		...
		for _,d in ipairs(list) do d[2] = d[2] *calc_character_diff(name0,names1[d[1]]) end
		table.sort(list,function(a,b) return a[2] > b[2] end)
