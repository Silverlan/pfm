		...
		for fcNameSrc, udmMappings in pairs(udmFcm:GetChildren()) do
			...
			if fcIdSrc == -1 then
				console.print_warning(
					"Retarget rig has invalid flex controller reference '"
						.. fcNameSrc
						.. "' for model '"
						.. srcMdl:GetName()
						.. "'! Ignoring..."
...
function ents.RetargetRig.Rig:GetFlexControllerTranslationTable()
	return self.m_flexTranslationTable
end

function ents.RetargetRig.Rig:ClearFlexControllerTranslation(flexCId0, flexCId1)
	if self.m_flexTranslationTable[flexCId0] == nil then
		return
	end
