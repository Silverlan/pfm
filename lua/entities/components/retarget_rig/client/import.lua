...
function ents.RetargetRig.Rig.find_viable_retarget_rig(mdlImpersonatee, mdlImpostor, skeleton, flexController)
	...
	if
		bestMatch ~= nil
		and (
			(skeleton == true and bestMatchPercentageBones >= matchThreshold)
			or (skeleton == false and bestMatchPercentage >= matchThreshold)
		)
	then
...
function ents.RetargetRig.Rig.find_and_import_viable_retarget_rig(
	mdlImpersonatee,
	mdlImpostor,
	skeleton,
	flexController
