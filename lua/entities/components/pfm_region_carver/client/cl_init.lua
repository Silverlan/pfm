...
function Component:SetCarveModel(ent,mdl)
	...
	if(actor ~= nil) then
		if(#mdl > 0) then
			...
			if(component == nil) then actorEditor:CreateNewActorComponent(actor,"pfm_region_carve_target",false) end
...
function Component:CarveModel(ent)
	...
	if(cuboidC == nil) then return end
	...
	for _,mg in ipairs(mdl:GetMeshGroups()) do
		for _,m in ipairs(mg:GetMeshes()) do
			for _,sm in ipairs(m:GetSubMeshes()) do
				...
				if(keepIds[uuid] == nil) then m:RemoveSubMesh(uuid)
				else table.insert(subMeshes,sm) end
			end
		end
	end
	...
	if(res ~= true) then console.print_warning("Failed to save carved model at location '" .. path .. "'!")
	else asset.reload(basePath,asset.TYPE_MODEL) end
...
function Component:Carve()
	...
	for ent,c in ents.citerator(ents.COMPONENT_PFM_ACTOR) do
		if(c:IsStatic()) then self:CarveModel(ent)
		elseif(ent:HasComponent("pfm_region_carve_target")) then
			...
			if(actor ~= nil) then
				...
				local pm = pfm.get_project_manager()
				local actorEditor = util.is_valid(pm) and pm:GetActorEditor() or nil
				if(util.is_valid(actorEditor)) then actorEditor:UpdateActorComponents(actor) end
			end
		end
	end
end
