...
function Component:FindLightSourceEntities()
	local it = ents.iterator({
		ents.IteratorFilterComponent(ents.COMPONENT_PFM_ACTOR),
		ents.IteratorFilterComponent(ents.COMPONENT_LIGHT),
	})
	local t = {}
	for ent in it do
			...
			if self.m_lightmapJob:IsSuccessful() then
				...
				if mat ~= nil then
					...
					if mat:Save() == false then
						...
						if actorC ~= nil then
							local pm = pfm.get_project_manager()
							pm:SetActorGenericProperty(
								actorC,
								"ec/light_map/lightmapMaterial",
								mat:GetName(),
								udm.TYPE_STRING
							)
						end
...
function Component:GenerateDirectionalLightmaps(preview, lightIntensityFactor)
	...
	local lightSources = {}
	for _, ent in ipairs(lightSourceEntities) do
