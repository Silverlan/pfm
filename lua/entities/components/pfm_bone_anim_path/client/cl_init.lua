...
function ents.PFMBoneAnimPath:Setup(boneName, session, filmClip)
	...
	if mdl == nil or animChannelTrack == nil or mdlCData == nil then
		return
	end
...
function ents.PFMBoneAnimPath:SetActiveTimeOffset(offset)
	if self.m_session == nil or self.m_filmClip == nil then
		return
	end
...
function ents.PFMBoneAnimPath:Render()
	if
		self.m_shaderLines == nil
		or self.m_shaderSprite == nil
		or self.m_bufPoints == nil
		or self.m_linePoints == nil
	then
		return
	end
