...
function Component:OnEntitySpawn()
	...
	if uv ~= nil then
		self.m_refUv = Vector2(vpData.cursorPos.x / vpData.width, vpData.cursorPos.y / vpData.height) - uv
	end
	...
	if cursorHitPos ~= nil then
		...
		local function find_best_candidate(fstart, fend, finterval, depth)
			...
			for i = fstart + finterval, fend - finterval, finterval do
				...
				local p
				if self.m_implicitPlane == nil then
					p = self:Raycast(pstart, -vector.UP, 10000)
				else
					local dir = -vector.UP * 10000
					local t = intersect.line_with_plane(
						pstart,
						dir,
						self.m_implicitPlane:GetNormal(),
						self.m_implicitPlane:GetDistance()
					)
					if t ~= false then
						p = pstart + dir * t
					end
