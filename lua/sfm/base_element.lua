...
function sfm.BaseElement:Initialize() end

function sfm.BaseElement:AddParent(parent)
	table.insert(self.m_parents, parent)
end
function sfm.BaseElement:GetParents()
	return self.m_parents
end

function sfm.BaseElement:FindParent(type, traversed)
	if self:GetType() == type then
		return self
	end
...
function sfm.BaseElement.RegisterGetter(elClass, name, settings)
	local getterName
	if settings ~= nil and settings.getterName ~= nil then
		getterName = settings.getterName
	else
		getterName = "Get" .. name:sub(1, 1):upper() .. name:sub(2)
	end
	elClass[getterName] = function(el)
		return el["m_" .. name]
	end
end
...
function sfm.BaseElement.RegisterSetter(elClass, name, settings)
	local setterName
	if settings ~= nil and settings.setterName ~= nil then
		setterName = settings.setterName
	else
		setterName = "Set" .. name:sub(1, 1):upper() .. name:sub(2)
	end
	elClass[setterName] = function(el, value)
		el["m_" .. name] = value
	end
end
	...
	if classData.properties ~= nil then
		...
		for name, propData in pairs(classData.properties) do
			...
			if class ~= nil then
				...
				if
					bit.band(propData[2], sfm.BaseElement.PROPERTY_FLAG_BIT_OPTIONAL) == 0
					and self["m_" .. name] == nil
				then
...
function sfm.BaseElement:GetName()
	return self.m_name
end
function sfm.BaseElement:SetName(name)
	self.m_name = name
end
function sfm.BaseElement:GetType()
	return self.m_type
end

function sfm.BaseElement:GetCachedElement(dmxEl, name)
	return self:GetProject():GetCachedElement(dmxEl, name)
end
function sfm.BaseElement:CacheElement(dmxEl, el, name)
	self:GetProject():CacheElement(dmxEl, el, name)
end
function sfm.BaseElement:CreatePropertyFromDMXElement(dmxEl, class, parent)
	return self:GetProject():CreatePropertyFromDMXElement(dmxEl, class, parent)
end
...
function sfm.BaseElement:LoadArrayValue(attr, class)
	...
	local o
	if type(elChild) == "userdata" then
		o = self:CreatePropertyFromDMXElement(elChild, class, self)
	else
		o = elChild
	end
	return o
...
sfm.register_element_type = function(name)
	...
	function class:__init(project, ...)
		if util.get_type_name(project) ~= "Project" then
			error(
				"Expected SFM project as argument for element of type '"
					.. name
					.. "', got "
					.. util.get_type_name(project)
					.. "!"
			)
		end
