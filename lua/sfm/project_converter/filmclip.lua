...
local function iterate_film_clip_children(converter, parent, node, parentName)
	local numTypes = 0
	if
		node:GetType() == "DmeProjectedLight"
		or node:GetType() == "DmeGameModel"
		or node:GetType() == "DmeCamera"
		or node:GetType() == "DmeGameParticleSystem"
	then
	...
	elseif node:GetType() == "DmeDag" then
		...
		if umdNode:GetType() ~= fudm.ELEMENT_TYPE_PFM_GROUP then
			umdNode = parent
		else
			parent:AddGroup(umdNode)
		end
		...
		pfm.log(
			"Unsupported film clip child type '"
				.. node:GetType()
				.. "' of parent '"
				.. node:GetName()
	...
	if numTypes > 1 then
		pfm.log(
			"Expected node '"
				.. node:GetName()
				.. "' to have a maximum of 1 type, but has "
				.. numTypes
		...
		for _, track in ipairs(trackGroup:GetTracks()) do
			for _, channelClip in ipairs(track:GetChannelClips()) do
				...
				if animSet ~= nil then
					...
					if actor ~= nil then
						pfm.log(
							"Renaming channel clip '"
								.. channelClip:GetName()
								.. "' of track '"
								.. track:GetName()
								.. "' of track group '"
								.. trackGroup:GetName()
								.. "' to name of associated actor '"
								.. actor:GetName()
								.. "'!",
							pfm.LOG_CATEGORY_SFM
						)
						...
						pfm.log(
							"Animation set '"
								...
								.. "' of track group '"
								.. trackGroup:GetName()
								.. "' has no associated actor! This clip will not be used!",
							pfm.LOG_CATEGORY_SFM,
							pfm.LOG_SEVERITY_WARNING
						)
					end
					...
					pfm.log(
						"Channel clip '"
							...
							.. "' of track group '"
							.. trackGroup:GetName()
							.. "' does not reference known animation set! The actor associated with this clip may not be animated properly!",
						pfm.LOG_CATEGORY_SFM,
						pfm.LOG_SEVERITY_WARNING
					)
				end
