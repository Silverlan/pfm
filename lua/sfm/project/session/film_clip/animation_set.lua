		...
		for _, attr in ipairs(elOperators) do
			...
			if elChild ~= nil then
				...
				elseif elChild:GetType() == "DmeRigParentConstraintOperator" then
					...
					pfm.log(
						"Unsupported animation set operator type '"
							.. elChild:GetType()
							.. "' for animation set '"
							.. self:GetName()
							.. "'! Operator will not be available!",
				...
				pfm.log(
					"Animation set '"
						.. self:GetName()
						.. "' has invalid operator value for operator '"
						.. tostring(attr)
						.. "'! Operator will not be available!",
					pfm.LOG_CATEGORY_PFM_CONVERTER,
					pfm.LOG_SEVERITY_WARNING
...
function sfm.AnimationSet:GetControls()
	return self.m_controls
end
function sfm.AnimationSet:GetTransformControls()
	return self.m_transformControls
end
function sfm.AnimationSet:GetOperators()
	return self.m_operators
end
