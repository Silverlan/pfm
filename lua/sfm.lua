...
sfm.source_units_to_pragma_units = function(units)
	return units
end
...
sfm.convert_source_fov_to_pragma = function(fov)
	if true then
		return math.horizontal_fov_to_vertical_fov(math.rad(fov), 4.0 / 3.0)
	end
...
sfm.convert_source_aperture_to_fstop = function(aperture)
	...
	local t0 = translationTable[i0]
	local t1 = translationTable[i1]
	local interpFactor = (t1.sfmAperture > t0.sfmAperture)
			and ((aperture - t0.sfmAperture) / (t1.sfmAperture - t0.sfmAperture))
		or 0.0
...
sfm.import_scene = function(fpath)
	...
	if f == nil then
		f = file.open_external_asset_file(fpath)
	end
	if f == nil then
		pfm.log(
			"Unable to import SFM project '" .. fpath .. "': File not found!",
